...
const ProductCard = ({ product, categories = [], onClick }) => {
  ...
  return (
    ...
    >
      ...
      {/* Product Info */}
      <div className="p-4">
        <h3 className="font-playfair font-bold text-lg text-charcoal-900 mb-1 line-clamp-1">
          {product.name}
        </h3>
        {product.material && (
          <p className="text-wood-600 text-sm mb-1">🪵 {product.material}</p>
        )}
        {product.dimensions && (
          <p className="text-wood-500 text-xs mb-3">📐 {product.dimensions}</p>
        )}
        {product.price && (
          <p className="font-semibold text-wood-700 mb-3">₹ {product.price}</p>
        )}
