...
const PrintCatalogPage = () => {
  ...
  // Add print-catalog-body class so @media print rules apply
  useEffect(() => {
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      ...
      {/* Sticky action bar — only visible on screen, hidden when printing */}
      <div className="no-print-btn sticky top-0 z-50 bg-white border-b border-gray-200 shadow-sm px-6 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-bold text-sm text-gray-800">Sagar Furniture — Product Catalog</p>
            <p className="text-xs text-gray-500">{products.length} products ready to print</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <button
          ...
          <button
            onClick={handlePrint}
            className="inline-flex items-center gap-2 bg-amber-700 hover:bg-amber-800 text-white font-semibold px-5 py-2 rounded-xl text-sm transition-all shadow-md hover:shadow-lg hover:-translate-y-0.5"
          >
      ...
      {/* Catalog Document */}
      <div className="bg-white mx-auto my-6 shadow-xl print-catalog-view" style={{ maxWidth: '210mm', minHeight: '297mm', padding: '16mm' }}>
